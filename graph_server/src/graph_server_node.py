...
class graph:
	...
	def __init__(self,file_name):
		...
		self.srv_remove_edge=rospy.Service("remove_edge",EdgeOper,self.remove_edge_srv);
		rospy.spin();
	...
	def add_edge(self,Edge_srv):
		cost=get_cost(Edge_srv.edge[0],Edge_srv.edge[1]);
		self.G.add_edge(Edge_srv.edge[0],Edge_srv.edge[1],weight=cost);

	def get_cost(self,nodeA,nodeB):
		return 0;
	def remove_edge_srv(self,Edge_srv):
		self.G.remove_edge(Edge_srv.edge[0],Edge_srv.edge[1]);
		return {'status':0}
